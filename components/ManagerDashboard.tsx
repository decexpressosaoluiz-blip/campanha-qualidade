...
const ManagerDashboard: React.FC<ManagerDashboardProps> = ({ stats, allCtes, onSelectUnit, onDateFilterChange, dateRange, lastUpdate, fixedDays }) => {
  ...
  const totalStats = useMemo(() => {
    return filteredStats.reduce((acc, curr) => ({
      ...
      recebido: acc.recebido + curr.recebido,
      ...
      comFoto: acc.comFoto + curr.comFoto,
      semFoto: acc.semFoto + curr.semFoto,
      semBaixaEntrega: acc.semBaixaEntrega + curr.semBaixaEntrega,
      totalCtes: acc.totalCtes + (curr.comMdfe + curr.semMdfe)
    }), { faturamento: 0, vendasDiaAnterior: 0, recebido: 0, meta: 0, projecao: 0, baixaNoPrazo: 0, baixaForaPrazo: 0, semBaixa: 0, comMdfe: 0, semMdfe: 0, comFoto: 0, semFoto: 0, semBaixaEntrega: 0, totalCtes: 0 });
  }, [filteredStats]);

  const deliveryStatsUniverse = useMemo(() => {
  ...
  const totalStatusEntrega = totalStats.comFoto + totalStats.semFoto + totalStats.semBaixaEntrega;
  ...
  const exportChart = async () => {
    ...
    const node = chartRef.current.querySelector('.recharts-responsive-container');
    if (!node) return;
    const dataUrl = await toPng(node as HTMLElement, { backgroundColor: '#ffffff', quality: 1, style: { borderRadius: '16px' } });
    const link = document.createElement('a');
    link.download = `faturamento-diario-${new Date().toISOString().split('T')[0]}.png`;
    link.href = dataUrl;
    link.click();
  };
      ...
      {activeTab === 'gerencial' ? (
        <div className="space-y-6 animate-fade-in">
               ...
               <div className="flex flex-col gap-4">
                 ...
                 <div className="grid grid-cols-3 gap-2 items-center text-center h-[90px]">
                   <div className="flex flex-col border-r border-gray-100 px-1">
                      <span className="text-2xl font-semibold text-green-700 leading-none">{deliveryStatsUniverse.noPrazo}</span>
                      <span className="text-[9px] font-bold text-green-600 bg-green-50/80 rounded-lg mt-3 py-1 uppercase">{deliveryStatsUniverse.pctNoPrazo.toFixed(0)}% OK</span>
                   </div>
                   <div className="flex flex-col bg-yellow-50/40 rounded-2xl px-2 py-4 border border-yellow-200/50 shadow-sm transform scale-105 z-10">
                      ...
                      <span className="text-2xl font-semibold text-yellow-700 leading-none">{deliveryStatsUniverse.semBaixa}</span>
                      <span className="text-[9px] font-bold text-yellow-600 mt-1 uppercase tracking-tighter">{deliveryStatsUniverse.pctSemBaixa.toFixed(0)}% PEND</span>
                   </div>
                   <div className="flex flex-col border-l border-gray-100 px-1">
                      <span className="text-2xl font-semibold text-red-700 leading-none">{deliveryStatsUniverse.foraPrazo}</span>
               ...
               <div className="flex flex-col gap-4">
                  <div className="grid grid-cols-3 gap-3 h-[100px] items-center text-center pt-6">
                     <div className="flex flex-col items-center justify-center border border-green-100 bg-green-50/30 p-2.5 rounded-xl">
                        <span className="text-2xl font-semibold text-green-700 leading-none">{totalStats.comFoto}</span>
                        <span className="text-[9px] font-bold text-green-600 uppercase mt-2">{(totalStats.comFoto / Math.max(1, totalStatusEntrega) * 100).toFixed(0)}% FOTO</span>
                     </div>
                     <div className="flex flex-col items-center justify-center border-2 border-red-200/60 bg-red-50/50 p-3 rounded-2xl shadow-sm transform scale-110 z-10 border-dashed">
                        <span className="text-2xl font-semibold text-red-700 leading-none">{totalStats.semFoto}</span>
                     ...
                     <div className="flex flex-col items-center justify-center border border-yellow-200/60 bg-yellow-50/40 p-2.5 rounded-xl">
                        <span className="text-2xl font-semibold text-yellow-700 leading-none">{totalStats.semBaixaEntrega}</span>
               ...
               <div className="flex flex-col gap-3 py-1 justify-center h-[145px]">
                  <div className="flex justify-between items-center p-4 rounded-2xl bg-green-50/40 border border-green-100 shadow-sm">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] font-semibold text-green-600 mt-2 uppercase">{(totalStats.comMdfe / Math.max(1, totalStats.comMdfe + totalStats.semMdfe) * 100).toFixed(0)}% COBERTURA</span>
                    </div>
                    <span className="font-semibold text-3xl text-green-600 leading-none">{totalStats.comMdfe}</span>
                  </div>
                  <div className="flex justify-between items-center p-4 rounded-2xl bg-red-50/40 border border-red-100 shadow-sm">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[9px] font-semibold text-red-600 mt-2 uppercase">{(totalStats.semMdfe / Math.max(1, totalStats.comMdfe + totalStats.semMdfe) * 100).toFixed(0)}% PENDENTE</span>
                    </div>
                    <span className="text-3xl font-semibold text-red-600 leading-none">{totalStats.semMdfe}</span>
                  </div>
               </div>
            </Card>
          </div>
          ...
          <Card title="FATURAMENTO CONSOLIDADO" icon={<DollarSign className="w-5 h-5 text-sle-primary opacity-40" />} className={`border-l-sle-primary transition-all duration-500 ${highlightedUnit ? 'ring-4 ring-sle-primary/20 scale-[1.01] bg-blue-50/30 shadow-xl' : ''}`}>
            <div className="flex flex-col md:flex-row items-center gap-10 py-3">
               <div className="flex-1 w-full text-center md:text-left">
                   ...
                   <span className="text-4xl font-semibold text-[#0F103A] tracking-tighter">{totalStats.faturamento.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}</span>
                   <p className="text-[11px] font-semibold text-gray-400 mt-2 uppercase tracking-tight">META GRUPO: {totalStats.meta.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}</p>
               </div>
               ...
               <div className="flex-[2] w-full bg-[#F1F3FB]/70 p-6 rounded-3xl border border-blue-100/50 shadow-inner">
                  <div className="flex justify-between items-end mb-4">
                     <div className="flex flex-col">
                        ...
                        <span className="font-semibold text-sle-primary text-3xl mt-1 tracking-tight">{totalStats.projecao.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}</span>
                     </div>
                  ...
                  <div className="relative w-full h-4 bg-white rounded-full overflow-hidden border border-blue-100">
                      ...
                      <div className={`absolute top-0 left-0 h-full ${generalPercentProj >= 100 ? 'bg-green-600' : 'bg-red-600'}`} style={{ width: `${Math.min((totalStats.faturamento / Math.max(1, totalStats.meta)) * 100, 100)}%` }}></div>
                  </div>
               </div>

               <div className="flex-1 w-full space-y-3">
                  <div className="bg-blue-50/80 border border-blue-100/40 rounded-2xl p-4 flex justify-between items-center">
                      <span className="text-[10px] font-bold text-blue-800 uppercase tracking-tighter">Vendas dia {(dateRange.end || lastUpdate.toISOString().split('T')[0]).split('-').reverse().slice(0,2).join('/')}</span>
                      <span className="text-2xl font-semibold text-[#2E31B4] leading-none">{totalStats.vendasDiaAnterior.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}</span>
                  </div>
          ...
          <div ref={chartRef} className="relative group">
            ...
            <button onClick={exportChart} className="absolute top-4 right-4 p-2 bg-white rounded-lg border border-gray-200 shadow-sm hover:bg-gray-50 text-gray-500 hover:text-sle-primary transition-all opacity-0 group-hover:opacity-100 z-10" title="Exportar Gráfico (PNG)"><Download className="w-4 h-4" /></button>
          </div>
        </div>
      ) : (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100 xl:col-span-2">
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
                <div className="p-5 border-b border-gray-50 bg-gray-50/20 flex items-center justify-between">
                  <h3 className="font-bold text-xs uppercase tracking-widest text-warning opacity-80">Ranking Entregas</h3>
                  <div className="group relative flex items-center">
                    <Info className="w-4 h-4 text-gray-300 cursor-help" />
